...
deploy_to_staging:
	...
	@${FUJI} deploy \
		...
		--branch staging \
		--cluster staging \
		--secrets DATABASE_URL=${SSM_STAGING}/DATABASE_URL,REDIS_URL=${SSM_STAGING}/REDIS_URL,NEXTAUTH_SECRET=${SSM_STAGING}/NEXTAUTH_SECRET,CALENDSO_ENCRYPTION_KEY=${SSM_STAGING}/CALENDSO_ENCRYPTION_KEY,GOOGLE_API_CREDENTIALS=${SSM_SHARED}/GOOGLE_API_CREDENTIALS,OTEL_EXPORTER_OTLP_ENDPOINT=/jaeger/staging/otlp_endpoint \
		--image ${IMAGE} \
